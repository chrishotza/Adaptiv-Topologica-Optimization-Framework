...
from experiments.online_topk_selector import _evaluate


def test_online_topk_selector_smoke(tmp_path: Path) -> None:
    ...
    assert 0.0 <= result["aggregate"]["probe_rate"] <= 1.0
    assert result["aggregate"]["mean_actions"] <= 2.0
    assert result["aggregate"]["paired_selector_minus_top1"]["better_graphs"] >= 0
