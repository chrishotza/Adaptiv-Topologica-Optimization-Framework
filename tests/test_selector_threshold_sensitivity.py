...
from experiments.selector_threshold_sensitivity import run


def test_selector_threshold_sensitivity_smoke(tmp_path: Path) -> None:
    ...
    assert all(cell["selector"]["graphs"] == 20 for cell in result["cells"])
    assert all(0.0 <= cell["probe_rate"] <= 1.0 for cell in result["cells"])
    assert all(cell["mean_actions"] <= 2.0 for cell in result["cells"])
