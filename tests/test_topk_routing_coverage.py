...
from experiments.topk_routing_coverage import run


def test_topk_routing_coverage_smoke(tmp_path: Path) -> None:
    ...
    assert set(result["aggregate"]["coverage"]) == {"1", "2", "3"}
    assert len(result["folds"]) == 4
    assert -1.0 <= result["aggregate"]["paired_top2_minus_top1_ceiling"]["mean"] <= 1.0
