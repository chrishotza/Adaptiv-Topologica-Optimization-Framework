...
from experiments.online_selector_local_predictor import LOCAL_KS, THRESHOLDS, run


def test_online_selector_local_predictor_smoke(tmp_path: Path) -> None:
    ...
    assert len(result["cells"]) == 1 + len(LOCAL_KS) * len(THRESHOLDS)
    assert all(cell["graphs"] == 20 for cell in result["cells"])
    assert all(0.0 <= cell["selector"]["probe_rate"] <= 1.0 for cell in result["cells"])
