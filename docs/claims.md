...
The BLOC product path exports node-to-block mappings as JSON, CSV, or TSV.

## Benchmark-qualified claims

The portfolio tests currently demonstrate that the ATOF portfolio can select the NetworkX Kernighan-Lin result on the Karate Club graph and reproduce a 10-edge cut under the stated two-way contract.

This is evidence for that graph and that configuration only.
...
Do not summarize those measurements as a universal winner without a defined corpus and protocol.
