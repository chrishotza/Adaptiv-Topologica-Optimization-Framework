...
def _load(path: Path) -> tuple[dict, list[dict]]:
    ...
    if payload.get("schema_version") != "1.0":
        raise ValueError("unexpected k=8 benchmark schema")
    if payload.get("k") != K:
        raise ValueError("expected k=8 benchmark")
    ...
    if len(payload.get("candidate_strategies", [])) != 8:
        raise ValueError("expected eight k=8 candidate strategies")

    records = payload.get("graph_manifest")
    if not isinstance(records, list) or len(records) != 20:
        raise ValueError("expected 20 graph records in k=8 graph manifest")
...
def run(path: Path) -> dict:
    ...
    result = {
        "schema_version": "1.0",
        "protocol": (
            "k=8 degree/hub oracle-free online selector with a five-seed fixed solver grid"
        ),
        ...
        "evidence_boundary": [
            ...
            "The candidate set, five-seed grid, and degree/hub routing configuration are frozen before held-out evaluation.",
