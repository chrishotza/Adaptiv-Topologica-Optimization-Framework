...
def _centroid_margin(router: LearnedTopologyRouter, topology: Mapping) -> float:
    ...
    if len(ranked) < 2:
        return float("inf")
    first = distances[ranked[0]]
    second = distances[ranked[1]]
    ...
    for row in training:
        for strategy, metrics in row["strategy_metrics"].items():
            values.setdefault(strategy, []).append(
                float(metrics["runtime_seconds"]) / row["edges"]
            )
    return {
        strategy: statistics.median(samples)
        for strategy, samples in values.items()
    }
...
def run_analysis(
    ...
    *,
    ...
    for test_corpus in corpora:
        ...
        for row in testing:
            ...
            if len(ranked) > 1 and margin < threshold:
                second = ranked[1]
                predicted_total = first_predicted + (
                    runtime_per_edge[second] * row["edges"]
                )
                if predicted_total <= budget_factor * first_predicted:
                    ...
                    observed_improvement = (
                        (first_cut - second_cut) / first_cut
                        if first_cut
                        else 0.0
                    )
            ...
            base = {
                ...
                "actual_runtime_seconds": float(
                    row["strategy_metrics"][first]["runtime_seconds"]
                ),
            }
        ...
        folds[test_corpus] = {
            ...
            "threshold": threshold,
            "summary": {
                "mean_relative_regret": _mean(
                    [row["relative_regret"] for row in fold_rows]
                ),
                "mean_runtime_seconds": _mean(
                    [row["actual_runtime_seconds"] for row in fold_rows]
                ),
                "probe_second_rate": _mean(
                    [row["actions"] > 1 for row in fold_rows]
                ),
            },
        }

    deltas = [
        candidate["relative_regret"] - base["relative_regret"]
        for candidate, base in zip(pooled, baseline)
    ]
    ...
    payload = {
        ...
        "summary": {
            "mean_relative_regret": _mean(
                [row["relative_regret"] for row in pooled]
            ),
            "mean_runtime_seconds": _mean(
                [row["actual_runtime_seconds"] for row in pooled]
            ),
            "probe_second_rate": _mean(
                [row["actions"] > 1 for row in pooled]
            ),
            "mean_actions": _mean([row["actions"] for row in pooled]),
        },
        "baseline_summary": {
            "mean_relative_regret": _mean(
                [row["relative_regret"] for row in baseline]
            ),
            "mean_runtime_seconds": _mean(
                [row["actual_runtime_seconds"] for row in baseline]
            ),
        },
        "paired_comparison": {
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/state_of_art/confidence_gated_allocation_latest.json"),
    )
