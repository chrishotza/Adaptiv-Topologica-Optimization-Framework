        ...
        for seed in SEEDS:
            for variant in VARIANTS:
                ...
                for penalty in PENALTIES:
                    ...
                    soft_runtime = time.perf_counter() - soft_started
                    ...
                    rows.append(
                        {
                            ...
                            "soft_runtime_seconds": soft_runtime,
                            "strict_accepted_moves": strict.accepted_moves,
                            "soft_accepted_moves": soft.accepted_moves,
        ...
        summaries.append(
            {
                ...
                "balanced_outputs": sum(row["soft_balance_error"] == 0.0 for row in subset),
                "mean_temporary_imbalance_steps": (
                    sum(row["temporary_imbalance_steps"] for row in subset) / len(subset)
                    if subset else 0.0
                ),
                "mean_accepted_move_delta": (
                    sum(
                        row["soft_accepted_moves"] - row["strict_accepted_moves"]
                        for row in subset
                    ) / len(subset)
                    if subset else 0.0
                ),
                "mean_runtime_seconds": (
                    sum(row["soft_runtime_seconds"] for row in subset) / len(subset)
                    if subset else 0.0
                ),
            }
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/soft-balance-ablation/latest.json"),
    )
