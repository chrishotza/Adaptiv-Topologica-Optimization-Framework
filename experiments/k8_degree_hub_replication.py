...
def _pad_isolated_nodes(graph, k: int) -> tuple[object, list[object]]:
    ...
    if target % k:
        target += k - target % k
    padded = graph.copy()
    padding = [f"__atof_k8_padding_{i}" for i in range(target - graph.number_of_nodes())]
    while any(node in padded for node in padding):
        padding = [f"__atof_k8_padding_{i}_x" for i in range(len(padding))]
...
def _graph_record(graph, corpus: str, name: str, *, seeds: tuple[int, ...] = SEEDS) -> dict:
    ...
    for seed in seeds:
        ...
        if missing:
            ...
            if missing:
                raise RuntimeError(
                    f"{corpus}/{name} seed={seed}: missing k=8 candidates {missing}; "
...
def run(output_path: str | Path, cache_dir: str | Path | None = None, *, seeds: tuple[int, ...] = SEEDS) -> dict:
    ...
    result = {
        "schema_version": "1.0",
        "protocol": "prospective k=8 degree/hub-only leave-one-corpus-out replication",
        ...
        "evidence_boundary": [
            "The k=8 strategy set is fixed before evaluation and excludes the k=2-only NetworkX Kernighan-Lin backend.",
            "Topology vectors are frozen to the five degree/hub features selected before this replication; routing remains IQR/L2.",
            ...
            "Tiny k=8 backend compatibility retries may add isolated padding nodes; padding nodes carry no edges and are removed before scoring the original graph.",
            "No held-out seed outcome is used to fit the router.",
            "The degree/hub-only feature subset is an experimental routing configuration; no production/default behavior changes.",
        ],
